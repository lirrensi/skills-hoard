...
def get_soffice_env() -> dict:
    """Get environment for LibreOffice (with LD_PRELOAD shim if needed)."""
    import os
...
def accept_changes(input_file: str, output_file: str) -> tuple[None, str]:
    """Accept all tracked changes in DOCX."""
    ...
    if not input_path.suffix.lower() == ".docx":
        return None, f"Error: Input file is not a DOCX: {input_file}"

    try:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(input_path, output_path)
    except Exception as e:
        return None, f"Error copying file: {e}"
    ...
    try:
        ...
        return None, f"Successfully accepted changes: {input_file} -> {output_file}"
